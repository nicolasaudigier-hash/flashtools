...
- Ne raconte jamais ta recherche ni ton raisonnement à voix haute ("je vais chercher...", "j'ai trouvé...", "voici le JSON"). Aucune phrase avant ou après. La toute première chose que tu écris dans ta réponse finale doit être le caractère { et la toute dernière le caractère }.
...
async function callClaude(promptKey) {
  ...
  // Avec la recherche web activée, Claude ajoute parfois du texte de
  // narration autour du JSON ("je vais chercher...", balises markdown...).
  // On isole donc le contenu entre la première { et la dernière } plutôt
  // que de supposer que toute la réponse est du JSON pur.
  const start = fullText.indexOf('{');
  const end = fullText.lastIndexOf('}');
  if (start === -1 || end === -1 || end <= start) {
    throw new Error(`Aucun objet JSON repérable pour "${promptKey}". Réponse complète (extrait) : ${fullText.slice(0, 500)}`);
  }
  ...
  try {
    ...
    throw new Error(`JSON invalide pour "${promptKey}" : ${e.message}. Extrait tenté : ${candidate.slice(0, 500)}`);
  }
